...
import { InstrumentSelector, Instruments } from "./instrument-selector";
...
export default function EditorPage() {
  ...
  const startLoop = async () => {
    ...
    Soundfont.instrument(
      acRef.current,
      selectedInstrument as unknown as Soundfont.InstrumentName
    ).then((inst: Soundfont.Player) => {
  ...
  const handleSaveInstrument = () => {
    const soundfont =
      Instruments[selectedInstrument]?.soundfont ?? selectedInstrument;
    const colors = [
      "#ef4444",
      "#3b82f6",
      "#10b981",
